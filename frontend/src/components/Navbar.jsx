...
export default function Navbar({ onOpenAuthModal, onReplayIntro }) {
  ...
  const [customNavLinks, setCustomNavLinks] = useState(null);
  ...
  const exploreLinks = customNavLinks
          ...
          <div className="flex items-center space-x-4 sm:space-x-6">
            ...
            <div className="relative font-mono hidden md:block">
              <button
                ...
                className="flex items-center space-x-2 bg-zinc-900 border border-zinc-800 hover:border-zinc-500 px-3 py-1.5 text-xs font-bold text-white rounded transition-colors shadow-sm"
                title="Explore All Pages"
              >
                <span>EXPLORE PAGES</span>
                <ChevronDown className={`w-3.5 h-3.5 text-zinc-400 transition-transform duration-200 ${navDropdownOpen ? 'rotate-180 text-emerald-400' : ''}`} />
              </button>

              {navDropdownOpen && (
                <div className="absolute left-0 mt-2 w-64 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 shadow-2xl p-2 z-50 rounded-xl backdrop-blur-xl">
                  <div className="text-[10px] font-mono text-slate-500 dark:text-zinc-500 px-3 py-1.5 font-bold uppercase tracking-wider border-b border-slate-200 dark:border-zinc-900 mb-1 flex items-center justify-between">
                    <span>EXPLORE PAGES</span>
                        ...
                        <Link
                          ...
                          className={`flex items-center justify-between px-3 py-2 text-xs font-medium rounded-lg transition-all ${
                            isActive
                              ...
                              : 'text-slate-800 hover:bg-slate-100 hover:text-slate-950 dark:text-zinc-300 dark:hover:bg-zinc-900 dark:hover:text-white'
                          }`}
                        >
                          <div className="flex items-center space-x-2.5">
                            <IconComponent className={`w-4 h-4 ${isActive ? 'text-white' : 'text-slate-600 dark:text-zinc-400'}`} />
                            <span className="font-sans font-semibold">{link.label}</span>
                          </div>
                          {link.badge && (
                            <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 bg-slate-100 text-slate-900 border border-slate-300 dark:bg-zinc-900 dark:text-emerald-400 dark:border-zinc-800 rounded uppercase">
                              {link.badge}
                            </span>
                          )}
                        </Link>
